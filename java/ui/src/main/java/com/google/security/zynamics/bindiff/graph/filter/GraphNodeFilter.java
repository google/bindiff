...
public class GraphNodeFilter {
  ...
  /**
   * Filter the nodes from a list that match a given filter criterium.
   *
   * @param nodes a {@code Collection} containing the nodes to be filtered
   * @param filterBy the criterium to filter by
   * @param resultSet a {@code Collection} of nodes that match the specified criterium. If {@code
   *     null}, this method only counts the matching nodes.
   * @return The number of nodes in the graph matching the filter criterium
   */
  private static <NodeType extends ZyGraphNode<?>> int internalFilterNodes(
      final Collection<NodeType> nodes,
      final Criterium filterBy,
  ...
  /**
   * Filter the nodes of an {@code AbstractZyGraph} that match a given filter criterium.
   *
   ...
   * @param graph the graph containing the nodes to be filtered
   * @param filterBy the criterium to filter by
   * @return a {@code Collection} of nodes that match the specified criterium. If {@code null}, this
   *     method only counts the matching nodes.
   */
  public static <NodeType extends ZyGraphNode<?>> List<NodeType> filterNodes(
      final AbstractZyGraph<NodeType, ?> graph, final Criterium filterBy) {
  ...
  /**
   * Count the nodes of an {@code AbstractZyGraph} matching a specified filter criterium.
   *
   * @param graph the graph containing the nodes to be filtered
   * @param filterBy the criterium to filter by
   * @return the number of nodes matching the criterium
   */
  public static int filterNodesCountOnly(
      final AbstractZyGraph<?, ?> graph, final Criterium filterBy) {
   ...
   */
  public static <NodeType extends ZyGraphNode<?>> List<NodeType> filterNodes(
      final Collection<NodeType> nodes, final Criterium filterBy) {
    final List<NodeType> resultSet = new ArrayList<>();
