...
if(EXISTS ${PostgreSQL_ROOT_DIR})
  ...
  if(WIN32)
    file(TO_NATIVE_PATH ${CMAKE_CURRENT_BINARY_DIR} _bin)
    file(TO_NATIVE_PATH ${PostgreSQL_SOURCE_DIR} _src)
    file(TO_NATIVE_PATH ${PostgreSQL_ROOT_DIR} _inst)
    ...
    file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/pg_install_headers.pl
               ...
               "Install::CopyIncludeFiles('${_inst}');")

    set(PostgreSQL_CONFIGURE_COMMAND cd /d "${_src}\\src\\tools\\msvc"
    COMMAND copy ${_bin}\\pg_config.pl
                 ${_src}\\src\\tools\\msvc\\config.pl
    COMMAND copy ${_bin}\\pg_install_headers.pl ${_src}
      COMMAND perl -pi.bak -e "s/MultiThreadedDLL/MultiThreaded/g"
      MSBuildProject.pm
      COMMAND perl mkvcbuild.pl)
    set(PostgreSQL_BUILD_COMMAND
      msbuild "${_src}\\pgsql.sln" /t:interfaces\\libpq
      /p:Configuration=Release /p:ConfigurationType=StaticLibrary /m)
    set(PostgreSQL_INSTALL_COMMAND cd /d "${_src}"
    COMMAND perl pg_install_headers.pl
    COMMAND if not exist "${_inst}\\lib" mkdir "${_inst}\\lib"
    COMMAND copy Release\\libpgport\\libpgport.lib "${_inst}\\lib"
    COMMAND copy Release\\libpq\\libpq.lib "${_inst}\\lib")
  elseif(UNIX)
    if(APPLE)
      set(_flags --build=i386-apple-darwin CFLAGS=-arch=i386 LDFLAGS=-arch=i386)
    else()
      set(_flags CFLAGS=-m32 LDFLAGS=-m32)
    endif()
    set(PostgreSQL_CONFIGURE_COMMAND
      "${PostgreSQL_SOURCE_DIR}/configure" ${_flags}
