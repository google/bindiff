...
void CallGraph::Read(const BinExport::Callgraph& proto,
                     const std::string& filename) {
  filename_ = filename;
  std::replace(filename_.begin(), filename_.end(), '\\', '/');
  ...
  for (int i = 0; i < proto.vertices_size(); ++i) {
    const BinExport::Callgraph::Vertex& vertex_proto = proto.vertices(i);
    VertexInfo& info = temp_vertices[i];
    info.address_ = static_cast<Address>(vertex_proto.address());
    temp_addresses[i] = static_cast<Address>(vertex_proto.address());

    // TODO(soerenme): What about thunk and imported?
    if (vertex_proto.function_type() == BinExport::Callgraph::Vertex::LIBRARY) {
      info.flags_ |= VERTEX_LIBRARY;
    }

    if (vertex_proto.has_real_name()) {
      info.flags_ |= VERTEX_NAME;
      info.name_ = vertex_proto.mangled_name();
      if (vertex_proto.has_demangled_name() &&
          !vertex_proto.demangled_name().empty()) {
        info.flags_ |= VERTEX_DEMANGLED_NAME;
        info.demangled_name_ = vertex_proto.demangled_name();
      }
    } else {
      // Provide a dummy name for display.
      info.name_ = vertex_proto.mangled_name();
  ...
  std::vector<std::pair<Graph::edges_size_type, Graph::edges_size_type>> edges;
  edges.reserve(proto.edges_size());
  std::vector<EdgeInfo> properties(proto.edges_size());
  for (int i = 0; i < proto.edges_size(); ++i) {
    // The edge may have connected string reference nodes which didn't get
    // loaded, that's why we have to check.
    const Address source_address =
        static_cast<Address>(proto.edges(i).source_function_address());
    const Address target_address =
        static_cast<Address>(proto.edges(i).target_address());
    const auto source = std::lower_bound(temp_addresses.begin(),
                                         temp_addresses.end(), source_address);
    const auto target = std::lower_bound(temp_addresses.begin(),
                                         temp_addresses.end(), target_address);
  ...
  Graph temp_graph(boost::edges_are_unsorted_multi_pass, edges.begin(),
                   edges.end(), properties.begin(), proto.vertices_size());
  std::swap(graph_, temp_graph);
  {
    VertexIterator i, end;
    int j = 0;
...
void CallGraph::DeleteVertices(Address from, Address to) {
  ...
  // TODO(soerenme): Bug: comments_ contains orphans now.
  // TODO(soerenme): Bug: IsDuplicate may contain wrong information for edges.
}
