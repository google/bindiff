  ...
  for (int i = 0; i < binja_instruction.branchCount; ++i) {
    ...
    switch (binja_instruction.branchType[i]) {
      ...
      case BNBranchType::CallDestination:
        // TODO(clichmann): Implement as IsPossibleFunction(branch_target,
        //                  modules))
        if constexpr (false) {
...
void AnalyzeFlowBinaryNinja(BinaryNinja::BinaryView* view,
  ...
  for (const auto& [address, function] : flow_graph->GetFunctions()) {
    ...
    // Skip autogenerated function symbols (e.g. thunks)
    if (bn_symbol_ref->IsAutoDefined() ||
        bn_symbol_ref->GetType() != BNSymbolType::FunctionSymbol) {
      continue;
    }

    function->SetName(bn_symbol_ref->GetRawName(),
                      bn_symbol_ref->GetFullName());
  }
